...
const Modal = ({activeId, setShowModal}) => {
    ...
    return (
        ...
        <div className="w-full h-full fixed top-2 left-0 z-10 bg-headingColor bg-opacity-40">
            ...
            <div>
               ...
               <p className="text-[15px] leading-7 text-smallTextColor ">
                ...
                <div className="mt-2 flex items-center gap-3 flex-wrap ">
                    <h4 className="text-headingColor text-[14px] text-[600]">
                    Technologies:
                    </h4>
                    {
                        portfolio.technologies.map((item, index)=> (<span key={index} className="bg-gray-200 py-1 px-2 rounded-[5px] text-[14px] leading-0">{item}</span>)) 
                    }

                </div>
                <a href={portfolio.siteUrl}>
                    <button className="bg-primaryColor text-white py-2 px-4 my-1 rounded-[8px] font-[400] hover:bg-headingColor ease-in duration-300">Live Site</button>
                </a>
            </div>
